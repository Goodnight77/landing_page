...
* **TQ 4-bit is competitive with SQ at half the storage.** On 7 of 10 datasets the gap to SQ is within 2 pp in either direction; `laion` is the closest of the wins, with TQ 4-bit ahead by 1.6 pp. On the other 3 datasets the gap is wider: TQ 4-bit *beats* SQ by 4.6 pp on `dbp-oai` and by 2.6 pp on `cohere`, while `arxiv-384` is the one dataset where it trails SQ, by 2.3 pp. The pattern is consistent: when SQ's int8-per-coordinate grid is mismatched with the embedding distribution, an adaptive 4-bit quantizer with anisotropy compensation does better, despite using half the bits.
